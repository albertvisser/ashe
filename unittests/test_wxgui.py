"""unittests for ./ashe/gui_wx.py
"""
from ashe import gui_wx as testee
...
class TestVisualTree:
    ...
    def _test_on_leftdclick(self, monkeypatch, capsys):
        """unittest for VisualTree.on_leftdclick
        """
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.on_leftdclick(evt=None) == "expected_result"
        assert capsys.readouterr().out == ("")
    ...
    def _test_on_rightdown(self, monkeypatch, capsys):
        ...
        """
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.on_rightdown(evt=None) == "expected_result"
        assert capsys.readouterr().out == ("")
...
class TestEditorGui:
    ...
    def _test_contextmenu(self, monkeypatch, capsys):
        """unittest for EditorGui.contextmenu
        """
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.contextmenu(arg=None) == "expected_result"
        assert capsys.readouterr().out == ("")
    ...
    def _test_ask_for_filename(self, monkeypatch, capsys):
        """unittest for EditorGui.ask_for_filename
        """
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.ask_for_filename(message, style) == "expected_result"
        assert capsys.readouterr().out == ("")
    ...
    def _test_get_search_args(self, monkeypatch, capsys):
        ...
        """
        testobj = self.setup_testobj(monkeypatch, capsys)
        assert testobj.get_search_args() == "expected_result"
        assert capsys.readouterr().out == ("")
